...
**Date**: 2025-11-15  
**Issue**: Duplication between `code` and `short_code` fields for storage
locations
...
**Question**: Why do we need both `code` and `short_code` if `code` is already
short enough (often ≤10 chars)?

**Current Spec Requirements**:

- Spec line 1316: "Short code is used in barcode generation and MUST follow same
  format constraints as location codes"
- Spec line 1317: "Labels MUST include human-readable text and barcode encoding
  using the short_code"
...
**Option 1** is recommended: Use `code` for labels if it's ≤10 chars, only
require `short_code` if `code` > 10 chars. This:
...
3. **Update spec**: Clarify when `short_code` is actually needed vs when `code`
   can be used
4. **Update implementation**: Modify validation and label printing logic
   accordingly
