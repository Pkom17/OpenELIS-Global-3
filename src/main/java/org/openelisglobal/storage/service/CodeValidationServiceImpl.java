...
/**
 * Implementation of CodeValidationService
 * Validates code format, length, and uniqueness
 */
@Service
public class CodeValidationServiceImpl implements CodeValidationService {
    ...
    private static final String CODE_PATTERN = "^[A-Z0-9][A-Z0-9_-]*$"; // Starts with letter/number, then alphanumeric/hyphen/underscore
    ...
    public CodeValidationResult validateFormat(String code) {
        ...
        if (normalized.length() > MAX_CODE_LENGTH) {
            return CodeValidationResult
                    .invalid(String.format("Code cannot exceed %d characters", MAX_CODE_LENGTH));
        }
        ...
        if (!normalized.matches(CODE_PATTERN)) {
            ...
            if (!normalized.matches("^[A-Z0-9_-]+$")) {
                return CodeValidationResult
                        .invalid("Code can only contain letters, numbers, hyphens, and underscores");
            }
    ...
    public CodeValidationResult validateUniqueness(String code, String context, String locationId, String parentId) {
        ...
        case "shelf":
            ...
            // TODO: Add findByCodeAndParentDevice method to StorageShelfDAO in implementation phase
            if (StringUtils.isBlank(parentId)) {
            ...
            // StorageShelf existingShelf = storageShelfDAO.findByCodeAndParentDevice(normalized, parentDevice);
            // For now, skip uniqueness check for shelf (will be added in implementation)
            break;

        case "rack":
            // Rack: unique within parent shelf
            // TODO: Add findByCodeAndParentShelf method to StorageRackDAO in implementation phase
            if (StringUtils.isBlank(parentId)) {
            ...
            // Placeholder: Will be implemented when DAO method is added
            // StorageRack existingRack = storageRackDAO.findByCodeAndParentShelf(normalized, parentShelf);
